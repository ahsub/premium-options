// ═══════════════════════════════════════════════════════════════
// PremiumOptions — Screener Engine v2
// Finnhub API + kuratierte Fallback-Liste
// S&P 500 + Dow Jones 30 + Nasdaq 100 (~600 Aktien)
// ═══════════════════════════════════════════════════════════════

// ── Vollständige Ticker-Listen ──────────────────────────────────
const DOW30 = [
...
const SP500_SAMPLE = [
  // Alle DOW30 + NASDAQ100 sind bereits enthalten
  // Zusätzliche S&P500-Sektoren (Ludwig-geeignet: $15-$80, stabile Branchen)
  // Financials
  'BAC','BLK','BRK.B','C','GS','JPM','MS','PNC','USB','WFC','AIG','AFL','MET','PRU','TRV',
  // Healthcare
  'ABT','ABBV','BMY','CVS','DHR','ISRG','JNJ','LLY','MDT','MRK','PFE','UNH','ZBH','BAX','BDX',
  // Consumer Staples
  'CL','CLX','GIS','HRL','HSY','K','KMB','KO','MDLZ','MKC','PEP','PG','SJM','SYY','TSN',
  // Industrials
  'CAT','DE','EMR','ETN','FDX','GE','HON','ITW','LMT','MMM','NOC','RTX','UPS','WM','XYL',
  // Energy
  'CVX','COP','EOG','KMI','MPC','OKE','OXY','PSX','SLB','VLO','XOM',
  // Utilities
  'AEP','D','DUK','ED','EIX','ETR','EXC','FE','NEE','NI','PEG','PPL','SO','WEC','XEL',
  // Materials
  'APD','CF','DOW','ECL','IFF','LIN','NEM','NUE','PKG','PPG','SHW',
  // Real Estate (REITs - Payout bis 90%)
  'AMT','ARE','AVB','CCI','DLR','EQR','O','PLD','PSA','SBAC','SPG','VICI','WY',
  // Technology (Kurs $15-$80)
  'CSCO','IBM','INTC','HPQ','HPE','QCOM','TXN','MSI','JNPR','NTAP','WDC',
  // Communication
  'CMCSA','DIS','EA','FOX','IPG','NFLX','OMC','PARA','T','VZ','WBD',
  // Consumer Discretionary
  'AZO','BBY','DHI','F','GM','LEN','LOW','MCD','NVR','PHM','ROST','TGT','TJX','WHR',
  // Additional mid-caps in Ludwig range
  'ALLY','ASO','BEN','BFH','BGS','CIEN','CLF','CMC','CNM','COIN','CSL',
  'DAL','DNB','DXC','EFC','ENVA','ETRN','GATX','GEO','GL','GNW',
  'HAL','HII','HRB','HST','IRM','JELD','KIM','KRC','LNC','LYV',
  'MAN','MOS','MPW','MSM','MTG','MTZ','NNN','NOG','OGN','OMF',
  'PAA','PBI','PCH','PK','PRGO','PTEN','RCM','RDN','RLI','RPM',
  'SBGI','SCS','SEE','SFM','SLM','SNV','SWK','SXT','SYF','TAP',
  'TDS','THG','TKR','TNL','TPH','TPC','TROX','TRMK','TRN','TWI',
  'UNM','VC','VFC','VIAC','VMI','WEN','WRB','WU','XRX','ZION'
];

// Dedupliziertes Gesamtuniversum
const FULL_UNIVERSE = [...new Set([...DOW30, ...NASDAQ100, ...SP500_SAMPLE])];

// ── Kuratierte Fallback-Liste (50 beste Ludwig-Kandidaten) ─────
// Vorberechnete Scores basierend auf bekannten Fundamentaldaten
const CURATED_50 = [
  ...
  {t:'INTC', price:19.8, pe:10, iv:38, payout:0,  fcf:false,revenue:false,weeklies:true, buyback:false,rsi:35, macd:false,support:18.5, resist:22.0, divYield:0.0, divGrowth:0.0, divStreak:0,  fcfCoverage:0.0},
  ...
  {t:'GS',   price:74.8, pe:14, iv:27, payout:18, fcf:true, revenue:true, weeklies:true, buyback:true, rsi:51, macd:true, support:72.0, resist:79.0, divYield:2.6, divGrowth:24.0,divStreak:6,  fcfCoverage:6.8},
  {t:'AXP',  price:71.3, pe:18, iv:25, payout:22, fcf:true, revenue:true, weeklies:true, buyback:true, rsi:54, macd:true, support:69.0, resist:76.0, divYield:1.3, divGrowth:15.2,divStreak:4,  fcfCoverage:4.5},
  ...
  {t:'QCOM', price:73.5, pe:16, iv:31, payout:38, fcf:true, revenue:true, weeklies:true, buyback:true, rsi:47, macd:true, support:71.0, resist:78.0, divYield:2.1, divGrowth:6.5, divStreak:22, fcfCoverage:3.0},
  ...
  {t:'DUK',  price:72.4, pe:19, iv:17, payout:68, fcf:false,revenue:true, weeklies:true, buyback:false,rsi:50, macd:false,support:70.0, resist:76.0, divYield:4.2, divGrowth:2.1, divStreak:18, fcfCoverage:0.8},
  {t:'SO',   price:68.1, pe:20, iv:16, payout:72, fcf:false,revenue:true, weeklies:true, buyback:false,rsi:51, macd:false,support:66.0, resist:71.0, divYield:3.8, divGrowth:2.8, divStreak:22, fcfCoverage:0.7},
  {t:'D',    price:45.2, pe:17, iv:18, payout:74, fcf:false,revenue:false,weeklies:true, buyback:false,rsi:47, macd:false,support:43.5, resist:48.0, divYield:5.1, divGrowth:1.5, divStreak:5,  fcfCoverage:0.6},
  ...
  {t:'K',    price:64.3, pe:17, iv:19, payout:60, fcf:true, revenue:false,weeklies:true, buyback:false,rsi:48, macd:false,support:62.0, resist:68.0, divYield:3.5, divGrowth:1.8, divStreak:14, fcfCoverage:1.9},
  ...
  {t:'HRL',  price:29.8, pe:18, iv:18, payout:60, fcf:true, revenue:false,weeklies:false,buyback:false,rsi:42, macd:false,support:28.5, resist:32.0, divYield:3.6, divGrowth:3.8, divStreak:58, fcfCoverage:1.6},
  ...
  {t:'LYB',  price:52.1, pe:9,  iv:28, payout:45, fcf:true, revenue:false,weeklies:true, buyback:true, rsi:41, macd:false,support:50.0, resist:56.0, divYield:5.8, divGrowth:3.5, divStreak:4,  fcfCoverage:2.0},
...
const RATE_LIMIT_MS = 1100; // 60/min = 1 call/sec + buffer

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

async function finnhubGet(endpoint, params, apiKey) {
...
// ── Cache Layer ───────────────────────────────────────────────
const CACHE_KEY = 'premium_options_screener_cache';
...
// ── Main Screener ─────────────────────────────────────────────
async function runFullScreener(params, apiKey, onProgress, onResult) {
  // Check cache first
  const cached = loadCache();
  if (cached && !params.forceRefresh) {
    onProgress({ phase: 'cache', pct: 100, msg: `Cache geladen (${cached.length} Aktien, max. 24h alt)` });
    return applyFilters(cached, params);
  }

  // No Finnhub key → use curated list
  if (!apiKey) {
    onProgress({ phase: 'fallback', pct: 100, msg: 'Kein Finnhub-Key → kuratierte 50er-Liste' });
  ...
  let processed = 0;
  let passed1 = []; // survived price filter
  let results = [];
  const BATCH = 5; // 5 parallel calls

  // ── Phase 1: Quote filter (price $15-$80) ──
  onProgress({ phase: 1, pct: 0, msg: `Phase 1/2: Kurs-Filter (${total} Ticker)...` });

  for (let i = 0; i < universe.length; i += BATCH) {
    const batch = universe.slice(i, i + BATCH);
    const promises = batch.map(sym =>
      finnhubGet('/quote', { symbol: sym }, apiKey)
        .then(q => ({ sym, price: q.c || 0, change: q.dp || 0 }))
        .catch(() => ({ sym, price: 0 }))
    );
    ...
    quotes.forEach(q => {
      if (q.price >= 15 && q.price <= 80) {
        passed1.push({ t: q.sym, price: q.price, priceChange: q.change });
      }
    });

    const pct = Math.round(processed / total * 50);
    onProgress({
      phase: 1, pct,
      msg: `Phase 1: ${processed}/${total} Ticker geprüft — ${passed1.length} im Kursbereich $15–$80`
    });
    await sleep(RATE_LIMIT_MS);
  }

  // ── Phase 2: Fundamentals ──
  onProgress({ phase: 2, pct: 50, msg: `Phase 2/2: Fundamentaldaten (${passed1.length} Kandidaten)...` });
  ...
  for (let i = 0; i < passed1.length; i += BATCH) {
    ...
    const promises = batch.map(async s => {
      try {
        const [basic, profile, metric] = await Promise.all([
          finnhubGet('/stock/metric', { symbol: s.t, metric: 'all' }, apiKey),
          finnhubGet('/stock/profile2', { symbol: s.t }, apiKey),
          finnhubGet('/stock/basic-financials', { symbol: s.t, metric: 'all' }, apiKey)
            .catch(() => null),
        ]);
        ...
        return {
          ...
          payout: m.payoutRatioAnnual ? m.payoutRatioAnnual * 100 : 0,
          divYield: m.dividendYieldIndicatedAnnual ? m.dividendYieldIndicatedAnnual : 0,
          divGrowth: m['5YDivGrowthRate'] || m['3YDivGrowthRate'] || 0,
          divStreak: 0, // nicht direkt via API → aus kuratierter Liste ergänzt
          fcfCoverage: m.freeCashFlowTTM > 0 ? (m.freeCashFlowTTM / Math.max(1, m.dividendPerShareAnnual * (p.shareOutstanding||0) * 1e6)) : 0,
          fcf: (m.freeCashFlowTTM || 0) > 0,
          revenue: (m['10YRevenueGrowthRate'] || m['5YRevenueGrowthRate'] || 0) > 0,
          weeklies: true, // assume true for large caps, will be corrected by option chain
          buyback: (m.roeTTM || 0) > 10, // proxy: high ROE often correlates with buybacks
          rsi: 50, // placeholder — no RSI from basic API, use chart tab
          macd: false, // placeholder
          support: s.price * 0.97,
          resist: s.price * 1.05,
          iv: estimateIV(m.beta || 1, m['52WeekPriceReturnDaily'] || 0),
    ...
    const batch_results = (await Promise.all(promises)).filter(Boolean);
    results.push(...batch_results);
    fundProcessed += batch.length;

    const pct = 50 + Math.round(fundProcessed / passed1.length * 50);
    onProgress({
      phase: 2, pct,
      msg: `Phase 2: ${fundProcessed}/${passed1.length} Fundamentaldaten geladen — ${results.length} Kandidaten`
    });
    await sleep(RATE_LIMIT_MS);
  }

  // Supplement with curated data for divStreak (not in API)
  const curatedMap = Object.fromEntries(CURATED_50.map(s => [s.t, s]));
  results = results.map(s => {
    const c = curatedMap[s.t];
    if (c) {
      s.divStreak = c.divStreak || 0;
      s.macd = c.macd;
      s.rsi = c.rsi;
    }
...
// ── IV-Schätzung aus Beta ─────────────────────────────────────
function estimateIV(beta, annualReturn) {
  // Grobe Schätzung: IV ≈ Beta × 20% + Basis
  const base = 15;
  const est = base + beta * 10 + Math.abs(annualReturn) * 0.1;
  return Math.min(70, Math.max(10, Math.round(est)));
}
...
function applyFilters(stocks, params) {
  return stocks.filter(s => {
    ...
    if (params.peMax && s.pe > params.peMax) return false;
    if (s.iv < (params.ivMin||20) || s.iv > (params.ivMax||50)) return false;
    ...
    if (params.needWeeklies && !s.weeklies) return false;
    if (params.needBuyback && !s.buyback) return false;
    // Dividend filters
...
// ── Score-Berechnung (identisch mit index.html) ───────────────
function calcScore(s) {
  let base = 0;
  if (s.revenue) base += 1;
  if (s.fcf)     base += 1;
  if ((s.pe||0) <= 20)  base += 1; else if ((s.pe||0) <= 25) base += 0.5;
  if ((s.payout||0) <= 60) base += 0.5;
  if (s.buyback) base += 0.5;
  if (s.weeklies) base += 0.5;
  if ((s.iv||0) >= 20 && (s.iv||0) <= 50) base += 1;
